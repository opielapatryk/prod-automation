...
kubectl exec -it $(kubectl get pods -l app=django -o jsonpath='{.items[0].metadata.name}') -- python manage.py makemigrations collector
kubectl exec -it $(kubectl get pods -l app=django -o jsonpath='{.items[0].metadata.name}') -- python manage.py migrate

echo "🔧 Removing login requirement from views..."
kubectl exec -it $(kubectl get pods -l app=django -o jsonpath='{.items[0].metadata.name}') -- bash -c 'cat > /app/collector/views.py << "EOF"
...
kubectl exec -it $(kubectl get pods -l app=django -o jsonpath='{.items[0].metadata.name}') -- python manage.py load_test_data

echo "✅ Deployment complete!"
echo ""
echo "📋 Available API URLs:"
echo "   - Admin Panel:          http://localhost:8000/admin/"
echo "   - Dashboard:            http://localhost:8000/api/collector/dashboard/"
echo "   - Machine Map:          http://localhost:8000/api/collector/map/"
echo "   - Machines Warnings:    http://localhost:8000/api/collector/machines/warnings/"
echo "   - Submit Telemetry:     http://localhost:8000/api/collector/telemetry/receive/"
echo "   - API Documentation:    http://localhost:8000/swagger/"
echo ""
echo "💡 To access the application locally, run: kubectl port-forward svc/django 8000:8000"